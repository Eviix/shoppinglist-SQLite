...
export default function App() {
  ...
  useEffect(() => {
    db.transaction(tx => {
      tx.executeSql('create table if not exists shoppinglist (id integer primary key not null, amounts int, title text);');
    }, null, updateList); 
  }, []);
  ...
  const saveItem = () => {
    db.transaction(tx => {
        tx.executeSql('insert into shoppinglist (amounts, title) values (?, ?);', [parseInt(amount), title]);    
      }, null, updateList
    )
  }
